...
/* ==========================================================================
   ROLETA VERTICAL FLUIDA (BENTO GRID - O QUE FAZEMOS)
   ========================================================================== */
const bentoList = document.querySelector('.bento-list');
if (bentoList) {
  // 1. Cria a máscara de desfoque (O efeito 3D da roleta)
  const wrapper = document.createElement('div');
  wrapper.style.height = '110px';
  wrapper.style.overflow = 'hidden';
  wrapper.style.position = 'relative';
  wrapper.style.maskImage = 'linear-gradient(180deg, transparent 0%, black 25%, black 75%, transparent 100%)';
  wrapper.style.webkitMaskImage = 'linear-gradient(180deg, transparent 0%, black 25%, black 75%, transparent 100%)';
  
  bentoList.parentNode.insertBefore(wrapper, bentoList);
  wrapper.appendChild(bentoList);

  // 2. Limpa o HTML antigo
  const items = Array.from(bentoList.querySelectorAll('li'));
  items.forEach(li => {
    li.textContent = li.textContent.replace('→', '').trim();
    li.className = 'bento-item'; 
  });

  // 3. Injeta o estilo com a setinha animada via CSS
  ...
  style.innerHTML = `
    ...
    .bento-item::before {
      content: '→';
      position: absolute;
      left: -32px; /* Posição inicial da seta */
      opacity: 0;
      color: var(--accent);
    ...
    .bento-item.highlight {
      color: var(--accent);
      transform: translateX(32px); /* Empurra o texto para a direita para dar espaço */
    }
    .bento-item.highlight::before {
      opacity: 1;
      transform: translateX(0); /* A seta entra suavemente */
    }
  `;
  document.head.appendChild(style);

  // 4. Motor de Rolagem Contínua (Hardware Accelerated)
  function updateHighlight() {
    const allItems = bentoList.querySelectorAll('li');
  ...
  setInterval(() => {
    const firstItem = bentoList.firstElementChild;
    const gap = parseInt(window.getComputedStyle(bentoList).gap) || 12;
    const scrollAmount = firstItem.offsetHeight + gap;

    // A) Ativa a transição suave para subir
    bentoList.style.transition = 'transform 0.8s cubic-bezier(0.25, 1, 0.5, 1)';
    bentoList.style.transform = `translateY(-${scrollAmount}px)`;

    // B) Sincroniza a iluminação do PRÓXIMO item durante a subida
    const allItems = bentoList.querySelectorAll('li');
    allItems[1].classList.remove('highlight');
    allItems[2].classList.add('highlight');

    // C) Reset invisível
    setTimeout(() => {
      bentoList.style.transition = 'none'; 
      bentoList.appendChild(firstItem); 
      bentoList.style.transform = 'translateY(0)'; 
    }, 800); 
